...
声明 role 参数（类型、是否必填、`choices`、默认值）。应用 role 时 Ansible 会在跑 `tasks/main.yml` **之前**自动校验，把坏输入变成清晰的提前失败——molecule converge 日志里能看到 `Validating arguments against arg spec 'main'`。这是惯用的输入校验 / 负向测试入口（见下文 ③）。
...
下面这些偏场景化，默认生成会让 starter 变臃肿，所以脚手架不生成、只讲清楚怎么接。

### ① 升级 / 脏状态测试：side_effect / prepare / cleanup

在 `test_sequence` 里 `idempotence` 之后插 `side_effect → converge → verify`，并写 `molecule/default/side_effect.yml` 制造漂移（改配置、停服务、装旧版本），再 converge 一次验证 role 能收敛回正确状态：
...
scenario:
  test_sequence: [dependency, create, converge, idempotence, side_effect, converge, verify, destroy]
```

`prepare`（converge 前置条件，如预装依赖、铺测试数据）和 `cleanup`（destroy 前的外部清理，如删云资源）同理，分别配 `prepare.yml` / `cleanup.yml` 并加进序列。
