...
**Dependency updates** (`dependency_updates`, **shared** across role+collection, default on): one **Renovate** config (`renovate.json`) replaces the old per-platform Dependabot — `enabledManagers` limited to `pep621` (uv: `pyproject.toml` + `uv.lock`, auto-detected via the lockfile) + `github-actions`, weekly schedule, `chore`/`ci` semantic-commit prefixes. The Forgejo track gets a scheduled `.forgejo/workflows/renovate.yml` (self-hosted via `renovatebot/github-action` over the runner's DooD socket; needs a `RENOVATE_TOKEN` secret and sets `RENOVATE_PLATFORM: forgejo` — the legacy `gitea` platform is dropping Forgejo support); GitHub relies on the hosted Renovate App reading `renovate.json` (no workflow). Both files are **static** (no `.jinja`) with conditional Jinja *path* names so the Actions `${{ }}` survive copier. The default Galaxy dep pins both bounds (`community.general: ">=11.0.0,<12.0.0"`) so a major bump is a reviewed Renovate PR, not silent.
...
**Scaffold self-test** (`.forgejo/workflows/test-template.yml`): the scaffold's own CI generates a role + a collection across the answer matrix and lints them (`lint-role` + `lint-collection`, both matrices covering every conditional path — docker/vagrant/adversarial/plugins/release variants — on push/PR), exercises `copier update` (`update-role` + `update-collection`: generate at the PR base → update to HEAD → assert no `.rej` → re-lint, on push/PR), plus gated smokes on main/dispatch: `smoke-role` (docker molecule), `smoke-role-vagrant` (containerized vagrant), `smoke-collection` (sanity — incl. the changelog test, since a default forgejo collection now ships `changelogs/` — + molecule), `smoke-collection-plugins` (sanity + units on a plugins collection). The builder image is resolved by a `resolve-image` job from the latest `builder/v*` tag (`git ls-remote ... | sort -V | tail -1`); only the host comes from `vars.BUILDER_REGISTRY`, so releasing a new image needs no variable bump. This is the regression net for template edits — extend it when you add template features.
