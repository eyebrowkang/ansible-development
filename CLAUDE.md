...
**Collection release automation** — two collection-only toggles mirroring the role's split, but the changelog is **antsibull-changelog** (fragment-based: `changelogs/fragments/*.yml`, validated by the `changelog` sanity test), not git-cliff/release-please — so the GitHub track is *tag-driven*, not release-please. `collection_release` (GitHub-gated, default on github/both) and `collection_forgejo_release` (Forgejo-gated; default on pure-forgejo, off for `both` to avoid double-release). Unlike the role tracks, collection release tags are **bare SemVer** (`0.3.0`, no `v` — the collection-ecosystem convention, and `ansible-galaxy` treats a `type: git` requirement's `version:` as a git ref, so the tag must equal the collection version for git installs to resolve). The release commit is prepared **before tagging**: bump `galaxy.yml`, `make changelog` (`antsibull-changelog release` folds fragments → `changelogs/changelog.yaml` + regenerates `CHANGELOG.md`, configured `output: md`), commit, tag that commit. The workflow (trigger `[0-9]*`) checks out **the tag itself** and verifies it (plain SemVer + galaxy.yml version == tag + HEAD exactly at the tag; immutable — no commit-back, never writes a branch or moves a tag), `ansible-galaxy collection build`, then — GitHub: `ansible-galaxy collection publish` to Galaxy (needs `GALAXY_API_KEY`) + a `gh` release; Forgejo: a release via the Forgejo API with the tarball attached (auto `GITHUB_TOKEN`), body from that version's changelog `release_summary`. GitHub `release.yml` is static; the Forgejo one is `.jinja` with `{% raw %}` around the steps (templated `image:`). `antsibull-changelog` joins the collection dev deps when either toggle is on; `make changelog` wraps it; the seed `CHANGELOG.md` keeps ansible-lint happy pre-first-release. The changelog infra is gated on `collection_release or collection_forgejo_release` (a `when`-skipped toggle still computes its default, so the `or` is safe).
